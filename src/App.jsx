...
const [items,setItems]=useState(null); // null = 還在載入中
const [sync,setSync]=useState(“idle”);
const saveTimer=useRef(null);

// 只在第一次載入時從 Firebase 讀一次
useEffect(()=>{
if(!db){ setItems(INITIAL_ITEMS); setSync(“offline”); return; }
...
}).catch(()=>{ setItems(INITIAL_ITEMS); setSync(“error”); });
},[]);

// 每次 items 變動就儲存到 Firebase（items 為 null 時跳過）
useEffect(()=>{
if(!items) return;
if(!db) return;
...
const syncMsg={saving:“同步中…”,saved:“☁️ 已同步”,error:“⚠️ 同步失敗”,offline:“📴 本地模式”,idle:””};
const syncColor={saving:”#a0aec0”,saved:”#2d6a4f”,error:”#c53030”,offline:”#e07800”,idle:“transparent”};

// 還在載入中，顯示 loading
if(!items) return (
<div style={{display:“flex”,alignItems:“center”,justifyContent:“center”,height:“100vh”,fontFamily:”‘Noto Sans TC’,sans-serif”,color:”#718096”,fontSize:15}}>
載入中…
</div>
);
